...
def hybrid_search(query, vector_db, es_bm25, k=5, semantic_weight=0.7, bm25_weight=0.3):
    """
    Perform hybrid search using both vector similarity and BM25.
    
    Args:
        query: The search query
        vector_db: ContextualVectorDB instance
    ...
    num_candidates = min(150, len(vector_db.metadata))
    
    # Semantic search
    semantic_results = vector_db.search(query, k=num_candidates)
    ...
    for doc_id, original_index in sorted_ids:
        # Find the document in the vector database
        doc = next((m for m in vector_db.metadata 
                   if m['doc_id'] == doc_id and m['original_index'] == original_index), None)
        
        if doc:
            results.append({
                "metadata": doc,
                "score": id_scores[(doc_id, original_index)],
                "in_semantic": (doc_id, original_index) in semantic_ids,
...
def main():
    ...
    while True:
        ...
        hybrid_results = hybrid_search(query, db, es_bm25, k=10)
