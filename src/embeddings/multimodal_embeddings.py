...
class MultimodalEmbeddings:
    """
    Generate contextual embeddings for document chunks with multimedia content and store them in Weaviate.
    """
    def __init__(self, weaviate_url="http://localhost:8081", class_name="ModalAIDocument", 
                 voyage_api_key=None, anthropic_api_key=None):
        """
        Initialize the multimodal embeddings generator.
    ...
    def load_processed_media(self, media_file="data/modalai_processed_media.json"):
        """
        Load processed media data from file.
        
        Args:
            media_file: Path to the processed media JSON file
        """
        try:
            if os.path.exists(media_file):
                ...
                for item in media_data:
                    ...
                    if media_id:
                        self.processed_media[media_id] = item
                        
                logger.info(f"Loaded {len(self.processed_media)} processed media items from {media_file}")
            else:
                logger.warning(f"Media file not found: {media_file}")
        except Exception as e:
            logger.error(f"Error loading processed media from {media_file}: {e}")

    def situate_context(self, doc, chunk, media_items=None):
        """
        Generate contextual information for a chunk using Claude, including media content.
        
        Args:
            doc: Full document content
            chunk: Content of the chunk
            media_items: List of media items associated with this document
        ...
        The document also contains these media items:
        <media_items>
        {media_content}
        </media_items>
        """
        ...
        If the chunk relates to any of the media items, include a brief reference to the relevant media content.
        Answer only with the succinct context and nothing else.
        """

        # Prepare media content summary if available
        media_summary = ""
        if media_items and len(media_items) > 0:
            ...
            for item in media_items:
                ...
                # Find the processed content if available
                processed_item = self.processed_media.get(item.get('path'))
                if processed_item:
                    # Extract text from the processed media
                    media_text = processed_item.get('text', '')
                    if media_text:
                        # Truncate if too long
                        if len(media_text) > 1000:
                            media_text = media_text[:1000] + "..."
                        
                        # Add details with extracted text
                        media_details.append(f"Type: {media_type}\nDescription: {item.get('alt_text', '')} {item.get('link_text', '')}\nExtracted content: {media_text}")
                else:
                    # Add basic details without processed content
                    media_details.append(f"Type: {media_type}\nDescription: {item.get('alt_text', '')} {item.get('link_text', '')}")
        ...
        try:
            ...
            if media_summary:
                messages[0]["content"].append({
                    "type": "text",
                    "text": MEDIA_CONTEXT_PROMPT.format(media_content=media_summary),
                    "cache_control": {"type": "ephemeral"}
                })
    ...
    def process_and_store(self, dataset_path="data/modalai_chunks.json", 
                         media_file="data/modalai_processed_media.json",
                         parallel_threads=5):
        """
        ...
        Args:
            ...
            media_file: Path to the processed media file
        ...
        # Load processed media
        self.load_processed_media(media_file)
        ...
        def process_chunk(doc, chunk):
            # Get media items for this document
            media_items = doc.get('media', [])
            
            # Generate contextual information for this chunk
            contextualized_text, usage = self.situate_context(doc['content'], chunk['content'], media_items)
...
def main():
    """Main entry point for multimodal embeddings."""
    import argparse
    
    parser = argparse.ArgumentParser(description="Generate multimodal embeddings for Modal AI documentation")
    parser.add_argument("--input", default="data/modalai_chunks.json", help="Input JSON file with chunks")
    parser.add_argument("--media-file", default="data/modalai_processed_media.json", help="Media file with processed content")
    ...
    processor = MultimodalEmbeddings(
    ...
    for i, result in enumerate(results):
        ...
        # Print media references if available
        media_refs = result['metadata'].get('media_references', [])
        if media_refs:
            print(f"Media References: {len(media_refs)} items")
            for media in media_refs[:2]:  # Show only first 2 media items
                print(f"  - {media.get('type', 'unknown')}: {media.get('alt_text', '')}")
